...
{
    void *buf;
    while (_os_unlikely(!(buf = malloc(size)))) {
...
{
    void *buf;
    while (_os_unlikely(!(buf = realloc(ptr, size)))) {
        _os_trace_temporary_resource_shortage();
    }
    return buf;
}
...
OS_NOINLINE
static uint32_t
_os_trace_blob_grow(_os_trace_blob_t ob, size_t hint)
{
    uint32_t size, minsize, used = ob->ob_len + !ob->ob_binary;
    if (_os_add_overflow(used, hint, &minsize)) {
        size = ob->ob_maxsize;
    } else if (_os_mul_overflow(ob->ob_size, 2, &size)) {
        size = ob->ob_maxsize;
    } else {
...
_os_trace_blob_add_slow(_os_trace_blob_t ob, const void *ptr, size_t size)
{
    ...
    if (avail < size) {
        if (ob->ob_size < ob->ob_maxsize) {
            avail = _os_trace_blob_grow(ob, size);
        }
        if (avail < size) {
